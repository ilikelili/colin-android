...
import com.windo.common.util.ShellUtils.CommandResult;
...
public class PackageUtils {
    ...
    public static final int uninstall(Context context, String packageName){
    	if(!PackageUtils.isSystemApplication(context)){
    		boolean isRoot = ShellUtils.checkRootPermission();
    		if(!isRoot){
    			return uninstallNormal(context, packageName) ? INSTALL_SUCCEEDED : INSTALL_FAILED_INVALID_URI;
    		}
    	}
    ...
    public static int unInstallSilent(Context context, String packageName) {
         ...
         **/
        StringBuilder command = new StringBuilder().append("pm uninstall ").append(packageName.replace(" ", "\\ "));
